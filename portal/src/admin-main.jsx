...
function Line({ rows, month, currency }) {
  ...
  const area = `${path} L ${points.at(-1).x} ${zeroY} L ${points[0].x} ${zeroY} Z`;
  return <><svg className="admin-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`Daily net collections ${money(values.reduce((a, b) => a + b, 0), currency)}`}><line x1={pad} x2={width - pad} y1={zeroY} y2={zeroY} className="admin-chart-grid" /><path d={area} className="admin-chart-area" /><path d={path} className="admin-chart-line" /></svg><div className="admin-chart-labels"><span>1</span><span>{Math.ceil(days / 2)}</span><span>{days}</span></div></>;
}
...
function Dashboard({ data, clinic, month, setMonth }) {
  ...
  const methods = Object.entries(data.payments.filter((row) => Number(row.amount) > 0).reduce((result, row) => { const key = row.payment_method || 'Other'; result[key] = (result[key] || 0) + Number(row.amount || 0); return result; }, {})).map(([label, value]) => ({ label, value: Math.round(value) }));
  ...
  const kpis = [
    ...
    ['Amount billed', money(metrics.billed, clinic.currency_code), 'Current net invoice value'], ['Net collection', money(metrics.collected, clinic.currency_code), `${change(metrics.collected, metrics.previousCollected)}% vs previous month`],
...
function Section({ section, data, clinic, profile, month, onRefresh }) {
  ...
  if (section === 'audit') return <><div className="admin-section-title"><h2>Audit and archived records</h2><p>Patient, appointment, clinical and financial changes are recorded with reasons and timestamps.</p></div><section className="admin-panel"><div className="admin-finance-note">Open a patient, appointment, visit or invoice to review field-level history, append-only chart corrections and invoice versions.</div></section></>;
